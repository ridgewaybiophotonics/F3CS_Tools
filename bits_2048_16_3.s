...
.LC0:
    ...
    .byte   1
	.text
.globl bits_2048_16_3
	.type	bits_2048_16_3, @function
bits_2048_16_3:
	pushl	%ebp
    movl	%esp, %ebp
	movl	8(%ebp), %eax #one
	...
	movl    20(%ebp), %eax     #   Pointer to remainder
	movq    (%eax), %xmm1
    movl    24(%ebp), %eax     #   Pointer to data[0]
...
.L5:
	...
	movl	24(%ebp), %eax    #   Get counter data address
	...
	incl	(%eax)
    movl	8(%ebp), %eax     #   Get one[i] array address
    movw    %cx, (%eax,%edx,1)     # copy the sum into memory (short)
	psrlw   $2, %xmm1      #   Left shift the data by two bits
    movdqa  %xmm1, %xmm2   #   Create a copy of counter data in xmm2
	psrldq  $1, %xmm2  #   shift xmm2 by one byte (time + 1)
	psubb   %xmm1, %xmm2   #   counter(t+1) - counter(t)
...
.L3:
    ...
    movl	24(%ebp), %eax    #   Get data address
	lddqu   (%eax, %edx, 4), %xmm1     #    Load data to xmm1
    psrldq  $8, %xmm1   #   shift xmm1 by 8 bytes
    movl    20(%ebp), %eax  #Add data to rem(ainder)
    movq    %xmm1, (%eax)

	popl	%ebp
	ret
	.size	bits_2048_16_3, .-bits_2048_16_3
